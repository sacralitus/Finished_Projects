# СКРИПТ ТРЕБУЕТ НАЛИЧИЯ МОДЕЛИ FASTTEXT (ссылка на модель - http://files.deeppavlov.ai/embeddings/ft_native_300_ru_twitter_nltk_word_tokenize.bin)
...
df.loc[df.role == "manager", "text_prepr"] = df[df.role == "manager"].text.apply(lambda x: apply_te(x, lan="ru"))
df.loc[df.role=="manager",  "text_prepr"] = df.text_prepr.str.replace("компани", ", Компани")
df["insight"] = ""
...
for dial_id in df.dlg_id.unique():
    ...
    counter = 0 # will count whether greeting&farewell are in stock 
    ...
    for ind, text in manager_dlg_df.text_prepr[:5].items():
        ...
        for span in doc.spans:
            ...
            if span.type == "PER" and any(i in text.lower() for i in ['меня', 'зовут', 'это']) and span.text != "Угу":
                ...
                info_dict['manager_name'][dial_id]=span.text
...
# конечный csv, для каждого диалога содержащий информацию об имени менеджера, названии компании, а также булево значение выполнения требования к менеджеру
